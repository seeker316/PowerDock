...
class DroneControl
{
    ...
    public:
        DroneControl : control(false)
        {
            command_pub = nh.advertise<plutodrone::PlutoMsg>("/drone_command", 1);
            key_sub = nh.subscribe("/input_key", 1, &DroneController::identifyKey, this);
            pid_sub = nh.subscribe("/pid_values", 1, &DroneController::arucoCallback, this);
        ...
        void identifyKey(const std_msgs::Int16::ConstPtr& msg) {
            ...
            switch (key) {
                ...
                case 95: control = false; ROS_INFO("Control off"); break;
            }
            command_pub.publish(cmd);
        }
};
